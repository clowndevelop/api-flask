...
class Fitness(db.Model):
    ...
    biceps_curl = db.Column(db.String(255),nullable=False)
    squat = db.Column(db.String(255),nullable=False)
    def serialize(row):
        return {
            "id" : str(row.id),
...
parser4ListFitness.add_argument('biceps_curl', type=str, help='biceps_curl', location='json', required=True)
parser4ListFitness.add_argument('squat', type=str, help='squat', location='json', required=True)
...
class NewFitness(Resource):
    ...
    def post(self):
        ...
        biceps_curl = args['biceps_curl']
        squat = args['squat']

        try:
            fitness = Fitness(pull_up=pull_up, push_up=push_up, biceps_curl=biceps_curl,squat=squat)
